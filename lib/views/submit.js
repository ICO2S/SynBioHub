...
function submitPost (req, res) {
  ...
  form.on('file', (name, file) => {
    console.log('file: name = ' + name + ' file = ' + JSON.stringify(file))
    ...
    switch (name) {
      case 'submitType':
        ...
        if (submissionData.overwriteMerge === 'unset') {
          ...
          submissionData.overwriteMerge = parseInt(submissionData.overwriteMerge, 10) + typeNum
        }
        console.log('submitType : ' + submissionData.overwriteMerge)
        break
      case 'overwrite_objects':
        if (submissionData.overwriteMerge === 'unset') {
          submissionData.overwriteMerge = 1
        } else {
          submissionData.overwriteMerge = parseInt(submissionData.overwriteMerge, 10) + 1
        }
        console.log('overwrite_objects : ' + submissionData.overwriteMerge)
        break
      case 'overwrite_merge':
        submissionData.overwriteMerge = value
        console.log('overwrite_merge : ' + submissionData.overwriteMerge)
        break
      case 'rootCollections':
        submissionData.collectionUri = value
        console.log('rootCollections : ' + submissionData.overwriteMerge)
        break
  ...
  form.on('close', () => {
    if (submissionData.filename && submissionData.filename !== 'unset') {
      console.log('close with = ' + JSON.stringify(submissionData))
      handleSubmission(req, res, submissionData)
    }
  })

  // console.log('PARSE FORM NOW')
...
async function sanitizeSubmission (submissionData) {
  ...
  if (submissionData.overwriteMerge === '0' || submissionData.overwriteMerge === '1') {
    ...
    if (!submissionData.collectionUri || submissionData.collectionUri === '') {
      ...
      tempStr = submissionData.collectionUri.replace(config.get('databasePrefix') + 'user/' + encodeURIComponent(submissionData.createdBy.username) + '/', '')
      submissionData.id = tempStr.substring(0, tempStr.indexOf('/'))
      submissionData.collectionId = submissionData.id + '_collection'
      submissionData.version = tempStr.replace(submissionData.id + '/' + submissionData.collectionId + '_collection/', '')
    }
  }

  const citationRegEx = /^[0-9]+(,[0-9]*)*$/
  if (submissionData.citations && submissionData.citations.trim() !== '' &&
!citationRegEx.test(submissionData.citations)) {
  ...
  if (submissionData.citations) {
    submissionData.citationArray = submissionData.citations.split(',').map(function (pubmedID) {
      return pubmedID.trim()
    }).filter(function (pubmedID) {
      return pubmedID !== ''
    })
  } else {
    submissionData.citationArray = []
  }
...
async function handleSubmission (req, res, submission) {
  ...
  if (submissionData.overwriteMerge === '1') {
    ...
    await sparql.deleteStaggered(removeQuery, graphUri)
    // Check if explorer is enabled, then hit the incrementalremovecollection endpoint
  ...
  let baseURI = config.get('databasePrefix') + 'user/' + encodeURIComponent(submissionData.createdBy.username) + '/' + submissionData.id
  let collectionURI = baseURI + '/' + submissionData.id + '_collection/' + submissionData.version
  ...
  const start = async () => {
    await asyncForEach(Object.keys(attachmentFiles), async filename => {
